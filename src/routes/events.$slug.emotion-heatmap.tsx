...
export const Route = createFileRoute("/events/$slug/emotion-heatmap")({
  ...
  head: () => ({
    meta: [
      { title: "Carte émotionnelle · Memento Live" },
      { name: "description", content: "Les pics d'émotion de la journée, minute par minute." },
      { property: "og:title", content: "Émotions live · Memento Live" },
    ...
    <div className="module-page">
      <header className="sticky top-0 z-40 border-b border-border/50 bg-background/90 backdrop-blur">
        <div className="mx-auto flex max-w-3xl items-center gap-3 px-4 py-4">
          ...
          <div className="flex-1">
            <h1 className="font-display text-xl">Carte émotionnelle</h1>
            <p className="text-xs text-muted-foreground">Signal live · captation acoustique</p>
          </div>
      ...
      <main className="mx-auto max-w-3xl space-y-6 px-4 py-6">
        ...
        <section className="space-y-2">
          {peaks.map((p) => (
            <div key={p.t} className="rounded-2xl border border-border/50 bg-card p-4">
